...
async function getOverseasValueKrw(isPaper: boolean): Promise<{ totalKrw: number; unrealizedPnlKrw: number }> {
  try {
    ...
    // 해외 보유종목 시가 + 원가 (v10.10.5c: unrealized PnL 정합성)
    const { rows: holdings } = await getPool().query(
      'SELECT quantity, avg_price, last_price, last_price_at FROM overseas_holdings WHERE quantity > 0 AND is_paper = $1',
      [isPaper],
    );
    ...
    for (const h of holdings) {
      ...
      marketValueUsd += qty * Number(h.last_price || 0);
      costBasisUsd += qty * Number(h.avg_price || 0);
    }
    ...
    const totalKrw = Math.round((marketValueUsd + cashUsd) * rate);
    const unrealizedPnlKrw = Math.round((marketValueUsd - costBasisUsd) * rate);
    return { totalKrw, unrealizedPnlKrw };
  } catch (err) {
    logger.warn(`해외 포트폴리오 조회 실패 (스냅샷): ${err}`, { component: 'SNAPSHOT' });
    return { totalKrw: 0, unrealizedPnlKrw: 0 };
  }
}
...
export async function runSnapshotJob(): Promise<void> {
  ...
  try {
    ...
    const overseas = await getOverseasValueKrw(isPaper);
    const totalValue = domesticValue + overseas.totalKrw;

    // v10.10.5c: 미실현PnL — 국내+해외 통합 (dashboard calc.ts와 정합)
    const domesticUnrealizedPnl = isPaper
      ? balance.totalEvalAmount - balance.purchaseCost
      : balance.totalProfitLoss;
    const snapshotUnrealizedPnl = domesticUnrealizedPnl + overseas.unrealizedPnlKrw;

    let dailyPnl = domesticUnrealizedPnl; // 폴백: 국내 미실현 손익
    ...
    try {
      ...
      if (todayStart && todayStart.total_value > 0) {
        ...
        const currentUnrealized = snapshotUnrealizedPnl;
    ...
    logger.info(
      `📸 스냅샷 저장 [${modeLabel}]: 총 ${totalValue.toLocaleString()}원 (국내 ${domesticValue.toLocaleString()} + 해외 ${overseas.totalKrw.toLocaleString()})`,
      { component: 'SNAPSHOT' },
    );
  } catch (error) {
  ...
  try {
    if (isPaper) {
      ...
      if (liveBalance.totalDeposit > 0 || liveBalance.totalEvalAmount > 0) {
        ...
        const liveOverseas = await getOverseasValueKrw(false);
        const liveTotalValue = liveDomestic + liveOverseas.totalKrw;
        const liveUnrealizedPnl = liveBalance.totalProfitLoss + liveOverseas.unrealizedPnlKrw;
        ...
        try {
          ...
          if (liveStart && liveStart.total_value > 0) {
            ...
            const unrealizedChange = liveUnrealizedPnl - prevUnrealized;
            let todayRealizedPnl = 0;
            try {
              const { rows: realizedRows } = await getPool().query(
                `SELECT COALESCE(SUM(realized_pnl), 0)::numeric AS total
      ...
      const paperOverseas = await getOverseasValueKrw(true);
      const paperTotalValue = paperDomestic + paperOverseas.totalKrw;
      const paperDomUnrealized = paperBalance.totalEvalAmount - paperBalance.purchaseCost;
      const paperUnrealizedPnl = paperDomUnrealized + paperOverseas.unrealizedPnlKrw;
      let paperDailyPnl = paperDomUnrealized;
      ...
      try {
        ...
        if (paperStart && paperStart.total_value > 0) {
          ...
          const unrealizedChange = paperUnrealizedPnl - prevUnrealized;
          let todayRealizedPnl = 0;
          try {
            const { rows: realizedRows } = await getPool().query(
              `SELECT COALESCE(SUM(realized_pnl), 0)::numeric AS total
      ...
      await insertSnapshot({
        ...
        unrealized_pnl: paperUnrealizedPnl,
