...
export async function sendBuyRecommendations(ctx: ExtendedAlertContext): Promise<void> {
  const {
    ...
    extendedAlertSentAt, updatedHoldings, techResults,
  } = ctx;
  ...
  for (const [code, h] of updatedHoldings) {
    ...
    if (pnl >= 3.0) holdingSells.push(`  🟢 *${code}* +${pnl.toFixed(1)}% @$${tech.price.currentPrice.toFixed(2)} | ${h.qty}주 $${posVal.toFixed(0)}(₩${(posVal * usdKrw / 10000).toFixed(1)}만) 비중${weightPct}%`);
    else if (pnl <= -5.0) holdingSells.push(`  🔴 *${code}* ${pnl.toFixed(1)}% @$${tech.price.currentPrice.toFixed(2)} | ${h.qty}주 손실$${(Math.abs(tech.price.currentPrice - h.avgPrice) * h.qty).toFixed(0)} → 손절검토`);
  }
  ...
  const alertLines = [
    ...
    ...alertTargets.map((t, i) => {
      ...
      const kPct = kellyResult.sampleCount >= 10 ? kellyResult.halfKelly : 0.20;
      const recSize = Math.min(portfolioValue * Math.min(kPct, 0.25), cash * 0.70);
  ...
  if (alertTargets.length > 0 || holdingSells.length > 0) {
    ...
    alertTargets.forEach(t => extendedAlertSentAt.set(t.code, Date.now()));
  }
}
...
export async function sendHoldingAlerts(ctx: {
  ...
  for (const [code, h] of updatedHoldings) {
    if (extendedAlertSentAt.has(`sell_${code}`) && now - (extendedAlertSentAt.get(`sell_${code}`) ?? 0) < 60 * 60_000) continue;
    const tech = techResults.find(t => t.code === code);
