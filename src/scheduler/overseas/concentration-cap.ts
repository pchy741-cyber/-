...
import { getHoldings, updateTradeState, cleanupPositionState } from './state.js';
...
export async function enforceConcentrationCap(params: {
  ...
  for (const [capCode, capHolding] of capHoldings) {
    ...
    const exec = await executeOverseasOrder(capCode, 'SELL', sellQty, capTech.price.currentPrice, capTech.exchange, `집중도 캡(${(posWeight * 100).toFixed(0)}% > 25%) — 20%로 강제 분산 매도`, capHolding.qty, capHolding.avgPrice, { isPaper });
    if (exec.submitted && exec.filledQty > 0) {
      const proceeds = exec.filledPrice * exec.filledQty * (1 - OVERSEAS_FEE_PCT);
      cash += proceeds;
      await updateTradeState({ code: capCode, exchange: capTech.exchange, qty: exec.finalQty, avgPrice: exec.finalAvgPrice, newCash: cash, isPaper });
      if (exec.finalQty <= 0) {
