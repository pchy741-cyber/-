...
export async function getTodayTradeStats(isPaper?: boolean) {
  ...
  try {
    const { rows } = await getPool().query(
      ...
      SELECT
        COUNT(*) AS total,
        COUNT(*) FILTER (WHERE o.side = 'SELL' AND o.stock_code ~ '^[0-9]') AS kr_sells,
        -- v10.2: 매도 수수료+세금(0.195%) 차감하여 매매내역과 일치
        COALESCE(SUM(
          CASE WHEN o.side = 'SELL' AND o.stock_code ~ '^[0-9]'
        ...
        COUNT(*) FILTER (WHERE o.stock_code !~ '^[0-9]' AND o.side = 'SELL') AS us_sells,
        COALESCE(SUM(
          CASE WHEN o.side = 'SELL' AND o.stock_code !~ '^[0-9]'
