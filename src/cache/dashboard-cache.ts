...
export function getDashCache(mode: string) {
  return _dashCacheByMode.get(mode) ?? null;
}
export function setDashCache(mode: string, data: unknown) {
  _dashCacheByMode.set(mode, { data, ts: Date.now() });
}
export function getDashBuildingByMode() {
  return _dashBuildingByMode;
}
...
export function invalidateCurrentModeCache(): void {
  _dashCacheByMode.delete('live');
  _dashCacheByMode.delete('paper');
}
...
export function invalidateModeCache(mode: string): void {
