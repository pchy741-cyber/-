...
function buildAdaptive(
  ...
  if (notes.length > 0) {
    logger.info(`⚙️ 전략 파라미터 자동최적화 [ATR ${atrPct.toFixed(2)}%]: ${notes.join(' | ')}`, { component: 'REGIME' });
  }
...
export async function fetchKospiRegime(): Promise<KospiRegime> {
  const _fallback: KospiRegime = { penalty: 0, boost: false, todayDown: false, flashCrash: false, adaptive: {}, atrPct: 1.0 };
  try {
    ...
    logger.info(`⚙️ 현재 ATR ${atrPct.toFixed(2)}% | SWING: threshold=${adaptive.SWING?.buyThreshold} TP=${adaptive.SWING?.takeProfitPct}% SL=${adaptive.SWING?.stopLossPct}%`, { component: 'REGIME' });
  ...
  try {
    ...
    const { rows } = await getPool().query(`
      ...
      WHERE o.side = 'SELL'
        ...
        AND tc.is_paper = $2
    `, [today.toISOString(), config.isPaper]);
