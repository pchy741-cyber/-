...
import { getInsiderScoreAdjustment, refreshInsiderData } from '../../automation/dart-insider.js';
import { getDisclosureScoreAdjustment, monitorDisclosures } from '../../automation/dart-monitor.js';
import { getCachedFundamentalScore, getCachedPiotroskiScore } from '../../automation/dart-research.js';
...
import { getNaverTrendScoreAdjustment, refreshNaverTrends, registerStockNames } from '../../automation/naver-trend.js';
import { checkNewsForStock } from '../../automation/news-sentinel.js';
import { getLastNewsCollectedAt } from '../../automation/news-collector.js';
import {
...
import { getLatestScores, getPool, logSystem } from '../../db/client.js';
...
export async function runTrackBPipeline(): Promise<TradeDecision[]> {
  ...
  try {
    ...
    const kstH = Number(_kstParts.find((p) => p.type === 'hour')?.value ?? '0');
    const kstM = Number(_kstParts.find((p) => p.type === 'minute')?.value ?? '0');
    ...
    await Promise.allSettled([
      Promise.race([
        refreshInsiderData(watchlistCodes),
        new Promise<void>((_, r) => setTimeout(() => r(new Error('timeout')), 3000)),
      ]),
      Promise.race([
        refreshNaverTrends(watchlistCodes),
        new Promise<void>((_, r) => setTimeout(() => r(new Error('timeout')), 3000)),
      ]),
    ]);
    ...
    // 개장 30분 내 하락장: 포트폴리오 스냅샷 없어도 즉시 KODEX 인버스 파킹 (개장 손실 누적 전 선제 대응)
    const _isOpeningDownDay = kstH === 9 && kstM < 30 && kospiRegime.todayDown;
    if (!parkState.isActive && !ctxIsPaper && kstH < 14) {
      const dt = await isPortfolioInDowntrend();
      const shouldPark = dt.downtrend || _isOpeningDownDay;
      const parkReason = dt.downtrend
        ? dt.reason
        : `개장직후 하락 감지 (09:${kstM.toString().padStart(2, '0')}, penalty=${kospiRegime.penalty})`;
      if (shouldPark) {
        // KODEX 200(069500)으로 시장 전체 기관 흐름 확인
        const kospiFlow = await getInvestorFlow('069500', 5).catch(() => null);
        const instBuying =
          kospiFlow && kospiFlow.institutionNet > 0 && (kospiFlow.trend === 'STRONG_BUY' || kospiFlow.trend === 'BUY');
        if (instBuying) {
          logger.info(
          ...
          logger.warn(`🛡️ 방어 파킹 진입 트리거: ${parkReason}`, { component: 'DEFENSE_PARK' });
          // 개장 하락장: crashSignal이 NONE이어도 CAUTION 최소 보장 → KODEX 인버스(114800) 매수 허용
          const effectiveCrashSignal =
            _isOpeningDownDay && crashSignal.level === 'NONE'
              ? { ...crashSignal, level: 'CAUTION' as const, score: Math.max(crashSignal.score, 20), reasons: [...crashSignal.reasons, '개장직후하락'] }
              : crashSignal;
          return buildDefenseParkEntryDecisions(
            openChains,
            livePrices,
            orderableCash,
            totalAssets,
            parkReason,
            effectiveCrashSignal,
          );
        }
      }
    }
    ...
    // 자동 DEFENSE 트리거: 급락 서킷브레이커 OR 개장30분내 하락 OR (하락장+당일하락+손실-1.5%+)
    // _isOpeningDownDay는 위 방어파킹 블록에서 이미 선언됨
    const autoShouldDefense =
      dbMode === 'SWING' &&
      !ctxIsPaper &&
      (kospiRegime.flashCrash ||
        _isOpeningDownDay || // 개장 30분 내 하락장 → 손실 누적 전 선제 방어파킹
    ...
    const autoShouldRevertFromSniper = dbMode === 'SNIPER' && highScoreCount === 0 && !autoShouldDefense;

    const effectiveModeRaw: StrategyMode = autoShouldDefense
      ? 'DEFENSE'
      : autoShouldSniper
        ? 'SNIPER'
    ...
    if (pendingPreMarketCodes.size > 0) {
      ...
      logger.info(`📋 동시호가 주문 종목 매수 차단: ${[...pendingPreMarketCodes].join(', ')}`, {
    ...
    const activePositionCount = openChains.filter(
      (c) => c.total_quantity > 0 && c.stock_code !== PARK_STOCK_CODE,
    ).length;
    const isMaxPositionsReached = !ctxIsPaper && activePositionCount >= allocRisk.maxPositions;
    if (isMaxPositionsReached) {
      logger.info(`🚫 최대 포지션 수 도달: ${activePositionCount}/${allocRisk.maxPositions} → 신규 매수 차단`, {
        component: 'TRACK_B',
      });
    }
    ...
    const isOpeningBlock = isRallyDay
      ? kstH === 9 && kstM < 10 && !getCtxIsPaper() && effectiveMode !== 'SCALPING'
      : isOpeningVolatility;
    const isLunchBlock = isLunchBan; // 랠리일이어도 마의 시간대 항상 적용 (93+ 극초고확신만 예외)
    if (isRallyDay && (isOpeningVolatility || isSwingEodRestricted)) {
      logger.info(`🚀 랠리일(KOSPI+1.5%+): 개장블록/종가우선 해제 → 적극 매수 허용 (마의시간대는 유지)`, {
        component: 'TRACK_B',
      });
    }
    // 뉴스 수집 끊김 감지 — 뉴스는 08:00/08:40 프리페치로 개장 전에 확보되어야 함
    // 09:00 개장 시점에 오늘 뉴스 없으면 즉시 차단, 장중 90분 초과 끊김도 차단
    const newsLastAt = getLastNewsCollectedAt();
    const isNewsStale =
      !ctxIsPaper &&
      kstH >= 9 && kstH < 15 && // 장중에만
    ...
    const entryTimingCheck =
      !ctxIsPaper && !blockNewBuys
    ...
    const adjustedScores = scores
      ...
      .map((s: any) => {
        ...
        const stock5dRet =
          stockCandles && stockCandles.length >= 6 && stockCandles[5].close > 0
        ...
        const effectiveKospiPenalty =
          kospiPenaltyAdj < 0 && stock5dRet != null
            ? relStrength >= 5.0
              ? Math.round(kospiPenaltyAdj * 0.2) // 80% 감면 (확실한 역행주)
              : relStrength >= 3.0
                ? Math.round(kospiPenaltyAdj * 0.5) // 50% 감면 (강한 역행주)
                : kospiPenaltyAdj
            : kospiPenaltyAdj;
    ...
    if (defenseFocusCodes) {
      logger.info(`🛡️ DEFENSE 집중: ${[...defenseFocusCodes].join(', ')} 2종목에만 신규 매수 허용`, {
        component: 'TRACK_B',
      });
    }
    ...
    if (!blockNewBuysFinal) {
      try {
        ...
        if (topCodes.length > 0) {
          ...
          logger.info(`📡 시그널 수집: ${marketSignals.size}/${topCodes.length}개 (상위${signalTopN})`, {
            component: 'TRACK_B',
          });
        }
      } catch (err) {
    ...
    const decisions = await technicalFallbackDecisions({
      ...
      // SCALPING 시간 게이트: 개장벨(09:00-09:14)에만 허용, 이후 97점 미만 차단
      allowScalpingBuys: effectiveMode !== 'SCALPING' || _isOpeningBell,
      livePrices,
      chartData,
      openChains,
      orderableCash,
      ...
      totalAssets,
      longTermHoldCodes: new Set(watchlist.filter((w) => (w as any).long_term_hold).map((w) => w.stock_code)),
      // LTH 종목은 당일 손절 재진입 차단에서 제외 — 하락장 탈출 후 재매수 허용
      lossBlockedCodes: (() => {
        const lthCodes = new Set(watchlist.filter((w) => (w as any).long_term_hold).map((w) => w.stock_code));
        const blocked = new Set([...recentLossCodes, ...todayRepeatStopCodes]);
        for (const c of lthCodes) blocked.delete(c);
        return blocked;
      })(),
    ...
    if (ctxIsPaper && effectiveMode !== 'BREAKOUT' && !blockNewBuysFinal) {
      try {
        const breakoutDecisions = await technicalFallbackDecisions({
          ...
          maxPositionKrw: adjMaxPositionKrw,
          totalAssets,
          longTermHoldCodes: new Set(filteredWatchlist.filter((w) => (w as any).long_term_hold).map((w) => w.stock_code)),
          // LTH 종목은 forceSell 후에도 BREAKOUT 재진입 허용
          lossBlockedCodes: (() => {
            const lthCodes = new Set(filteredWatchlist.filter((w) => (w as any).long_term_hold).map((w) => w.stock_code));
            const blocked = new Set([...recentLossCodes, ...todayRepeatStopCodes]);
            for (const c of lthCodes) blocked.delete(c);
            return blocked;
          })(),
    ...
    const actionable = await applyDecisionFlow({
      ...
      kospiRegime: {
        ...
        todayDown: kospiRegime.todayDown,
        adamKhooBullish: kospiRegime.adamKhoo?.bullish,
      },
      resolvedSl,
      resolvedTp,
      orderableCash,
    ...
    if (ctxIsPaper) {
      import('../../risk/paper-balance.js').then((m) => m.checkAndRefillPaper()).catch(() => {});
    }
    // 황금비율 자동 조정 (1시간 rate limit)
    import('../../automation/regime-allocator.js').then((m) => m.autoTuneRegimeWeights()).catch(() => {});
    ...
    if (isParkingException) {
      ...
      if (blocked > 0) {
        logger.info(`🛡️ 파킹 예외 필터: ${blocked}건 BUY 차단 (예외 종목 ${parkExceptCodes.size}개 통과)`, {
          component: 'TRACK_B',
        });
      }
      return parkFiltered;
