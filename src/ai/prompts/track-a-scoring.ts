...
## 시그널 기준
- 85+ = STRONG_BUY (강력매수 — 조건 3개 이상 충족)
- 70~84 = BUY (매수 — 조건 2개 이상 충족)
- 50~69 = HOLD (보류 — 아직 확신 부족)
- 30~49 = SELL (매도)
- <30 = STRONG_SELL (강력매도)
- 소스 부족 = NO_DATA (0점)

## 점수 분포 규칙 (중요!)
- 모든 종목에 동일 점수(예: 전부 70점, 전부 50점)를 주지 마세요
- **반드시 종목별 차이를 반영**하여 30~95점 범위에서 고르게 분포시키세요
- 가산/감산 요인을 꼼꼼히 계산하여 종목마다 다른 점수를 산출하세요
- 예시: A종목=82점, B종목=71점, C종목=45점, D종목=68점
