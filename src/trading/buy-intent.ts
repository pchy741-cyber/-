...
const TTL_MS = 10 * 60 * 1000; // 10분 — 미체결 주문도 보호

const _intents = new Map<string, BuyIntent>();

/** 매수 의도 등록 시도. 성공하면 true, 이미 다른 전략이 등록했으면 false */
export function registerBuyIntent(stockCode: string, source: string): boolean {
  cleanup();
  const existing = _intents.get(stockCode);
  if (existing) {
    logger.info(
      `🔒 매수의도 충돌: ${stockCode} — ${source} 차단 (이미 ${existing.source}가 ${Math.round((Date.now() - existing.registeredAt) / 1000)}초 전 등록)`,
      { component: 'BUY_INTENT' },
    );
    return false;
  }
  _intents.set(stockCode, { stockCode, source, registeredAt: Date.now() });
  return true;
}

/** 매수 의도 해제 (주문 실패 또는 취소 시) */
export function releaseBuyIntent(stockCode: string): void {
  _intents.delete(stockCode);
}

/** 해당 종목에 이미 매수 의도가 있는지 확인 (읽기 전용) */
export function hasBuyIntent(stockCode: string): boolean {
  cleanup();
  return _intents.has(stockCode);
}

/** 현재 등록된 모든 매수 의도 코드 */
export function getAllBuyIntentCodes(): Set<string> {
  cleanup();
  return new Set(_intents.keys());
}

/** TTL 만료된 항목 정리 */
function cleanup(): void {
  const now = Date.now();
  for (const [code, intent] of _intents) {
    if (now - intent.registeredAt > TTL_MS) {
      _intents.delete(code);
    }
  }
}
