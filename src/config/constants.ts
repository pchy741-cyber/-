...
export const STRATEGY_PARAMS = {
  SWING: {
    // ┌─ 수익 구조 ─────────────────────────────────────────────────────────┐
    // │ 1차매수 → -2% 물타기(평단↓1%) → 손절 -5% / 익절 +4%              │
    // │ 손익비 4:5 / 물타기 후 실효 손익비 개선 / 손익분기 승률 ~44%       │
    // │ splitCount=2: 1차 50% → 물타기 50% → 평균단가 최적화              │
    // └────────────────────────────────────────────────────────────────────┘
    buyThreshold: 45,
    // 신호 진입 점수: 45점 (AI+기술 복합 신호 — 더 많은 유효 기회 포착)
    splitCount: 2,
    averageDownPct: -2.0,
    // 물타기: -2% (손절 -5% 전에 반드시 발동 — 평균단가 ~1% 낮춤)
    maxAveragingCount: 1,
    takeProfitPct: 6.0,
    // 1단계 익절: +6.0% → 50% 매도, 잔여 트레일링 스톱 (R:R=1.2, 게이트 통과)
    takeProfitRatio: 0.5,   // 50% 부분 매도 → 잔여 트레일링
    stopLossPct: -5.0,
    // 손절: -5.0% (물타기 공간 확보 — 일 노이즈 2~3% + 물타기 버퍼 2%)
    maxHoldingDays: 7,
