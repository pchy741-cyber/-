...
export const STRATEGY_PARAMS = {
  SWING: {
    ...
    // └────────────────────────────────────────────────────────────────────┘
    buyThreshold: 55,
    // 신호 진입 점수: 55점 (품질 우선 — 승률 개선, 거래 횟수 줄고 수익률 ↑)
    splitCount: 2,
    averageDownPct: -3.0,
    // 물타기 1차: -3% (손절 -4% 전 충분한 버퍼 확보)
    maxAveragingCount: 2,
    // 물타기 2차: -4.5% (1차 -3% + 추가 하락 시 한 번 더)
    takeProfitPct: 7.0,
    // 익절: +7.0% → 50% 매도, 잔여 트레일링 스톱 (R:R=1.75, 손익분기 36.4%)
    takeProfitRatio: 0.5,   // 50% 부분 매도 → 잔여 트레일링
    stopLossPct: -4.0,
    // 손절: -4.0% (물타기 2회 허용 후 이 선 이탈 시 정리)
    maxHoldingDays: 7,
