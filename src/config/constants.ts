...
export const SCHEDULE = {
  // Track A: 핵심 4회 (v16.1: 6→4회, 토큰 33% 절약 — 09:30/10:00 제거, 09:00에 시초가 반영 충분)
  TRACK_A_CRON: ['30 7 * * 1-5', '0 9 * * 1-5', '30 12 * * 1-5', '0 18 * * 1-5'], // 07:30, 09:00, 12:30, 18:00 KST 평일
...
export function getDynamicDomesticTpSl(h: DomesticTpSlHints): {
  ...
  if (h.score >= 93) {
    ...
    tp = 8.0;
    // v21: -4.5%→-5.5%. 실전 14일: 42% 손절률, 평균 5.34h 손절 → 노이즈 필터 더 확대.
    sl = -5.5;
  } // 고확신: 1.45:1 R:R (노이즈 손절 최소화 → 승률 개선 우선)
  else if (h.score >= 80) {
    tp = 7.0;
    // v21: -4.2%→-5.0%. 실전 분석: 이 구간 손익분기 미달 → SL 확대로 생존율 높임.
    sl = -5.0;
  } // 중확신: 1.4:1 R:R
  else {
    tp = 6.0;
    // v21: -4.0%→-4.5%. 저확신도 노이즈 필터 확대.
    sl = -4.5;
  } // 저확신: 1.33:1 R:R
...
export const OVERSEAS = {
  ...
  AI_INTERVAL_MS: 10 * 60_000, // v14: AI 호출 간격 10분 (기회 포착 빈도 +50%)
...
export const MDD_LIMIT = {
  ...
  PAPER: 80, // v21: 40→80% (index.ts paperRisk.mddLimit=80과 통일, Paper는 데이터 수집 우선)
} as const;
