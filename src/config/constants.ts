...
export function getDynamicDomesticTpSl(h: DomesticTpSlHints): {
  ...
  if (h.score >= 93) {
    ...
    tp = 8.0;
    // v20: -3.5%→-4.5%. 30일 데이터: 이 구간 33건 중 42%가 손절, 승률 33%/평균 -1.60%.
    // 손절까지 걸린 시간이 전 구간 중 가장 짧아(5.34h) 진입 초반 정상 변동성을 손절로 오인 중이었음.
    sl = -4.5;
  } // 고확신: 1.78:1 R:R (노이즈성 손절 감소 우선)
  else if (h.score >= 80) {
    tp = 7.0;
    // v20: -3.5%→-4.2%. 이 구간 106건 중 승률 42%/평균 -0.06%(사실상 손익분기 미달)로 미조정.
    sl = -4.2;
  } // 중확신: 1.67:1 R:R (기존 진입 기준선)
  else {
    tp = 6.0;
    sl = -4.0;
  } // 저확신: 1.5:1 R:R
